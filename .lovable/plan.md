...
- Full recipe (ingredients, steps, nutrition breakdown) with a macro comparison panel: *Original vs Your target*.
...
- **Stack:** TanStack Start, Lovable Cloud (auth + Postgres), Lovable AI Gateway (Gemini) for substitution reasoning.
...
- **Substitution pipeline:** Gemini receives the ingredient list + macro delta needed and returns structured JSON swaps. For each swap, server function looks up the substitute in Spoonacular's ingredient DB to get real per-100g macros, recomputes deltas, and returns verified suggestions to the client.
- **Scaling:** pure math on Spoonacular's per-ingredient nutrition — multiply each ingredient amount by `target_kcal / original_kcal`.
- **Database tables (Lovable Cloud):** `profiles` (user_id, display_name), `macro_goals` (user_id, kcal, protein_g, carbs_g, fat_g), `saved_recipes` (user_id, spoonacular_id, title, image, custom_ingredients jsonb, applied_swaps jsonb, target_macros jsonb). RLS: users only see their own rows.
- **Secrets needed after approval:** `SPOONACULAR_API_KEY` (free tier at spoonacular.com/food-api). Lovable AI key is auto-provisioned.
