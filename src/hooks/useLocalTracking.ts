...
export function useLocalTracking() {
  ...
  const setAppleHealthSummary = useCallback(
    (date: Date, summary: AppleHealthSummary | undefined) => {
      setDiary((prev) => {
        const key = keyForDate(date);
        const day = prev[key] ?? createEmptyTrackingDay();
        return { ...prev, [key]: { ...day, appleHealthSummary: summary, garminSummary: undefined } };
      });
    },
    [],
  );
  ...
  return {
    ...
    /** @deprecated use setAppleHealthSummary */
    setGarminSummary: setAppleHealthSummary,
  };
}
