...
export const APIRoute = createAPIFileRoute("/api/food/search")({
  GET: async ({ request }) => {
    const url = new URL(request.url);
    const query = url.searchParams.get("q") ?? "";
    if (!query.trim()) return Response.json({ results: [] });
    const parsed = InputSchema.safeParse({ query });
    if (!parsed.success) return Response.json({ results: [] }, { status: 400 });
    ...
    // Interleave USDA and OFF results, deduping by name
