...
type FdcSearchResponse = { foods?: FdcFood[] };
...
export const APIRoute = createAPIFileRoute("/api/food/estimate")({
  POST: async ({ request }) => {
    let body: unknown;
    try {
      body = await request.json();
    } catch {
      return Response.json({ estimate: null, error: "Invalid JSON body" }, { status: 400 });
    }

    const parsed = InputSchema.safeParse(body);
    if (!parsed.success) {
      return Response.json({ estimate: null, error: parsed.error.message }, { status: 400 });
    }
    ...
    if (!res.ok) {
      return Response.json(
        { estimate: null, error: `USDA nutrition lookup failed (${res.status})` },
        { status: 502 },
      );
    }
    ...
    if (!food) {
      return Response.json({
        estimate: null,
        error: "No USDA nutrition match found.",
      });
    }

    return Response.json({ estimate: buildEstimate(food, name, quantity, unit), error: null });
  },
});
