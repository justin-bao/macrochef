...
import { z } from "zod";
import type { AIFoodItem } from "./estimate-ai.post";

// Re-implement helpers locally so this file has no cross-file imports
// (Nitro bundles each handler independently).
...
const InputSchema = z.object({
  ...
  mode: z.enum(["meal_photo", "nutrition_label"]).default("meal_photo"),
  context: z
    .object({
...
const SEARCH_USDA_TOOL = {
  ...
  function: {
    name: "search_usda",
    description: "Search USDA FoodData Central for whole or unprocessed foods. Returns top 5 matches with per-100g macros.",
...
const SEARCH_OFF_TOOL = {
  ...
  function: {
    ...
    description: "Search Open Food Facts for packaged or branded food products. Returns top 5 matches with per-100g macros.",
    parameters: { type: "object", properties: { query: { type: "string" } }, required: ["query"] },
  },
};
...
export default defineEventHandler(async (event) => {
  ...
  const { imageDataUrl, mode, context, useToolCalling } = parsed.data;
  ...
  const isLabel = mode === "nutrition_label";
  const withTools = useToolCalling !== false && !isLabel;
...
3. For each ingredient, call the appropriate tool (max 10 calls total):
   - search_usda for whole/unprocessed foods.
...
4. After reviewing results, decide:
   ...
   - If results are poor, return a single item for the whole dish with confidence "low".
...
Identify the meal and estimate macros from your own knowledge. Break into constituent ingredients with realistic portions. If uncertain, return a single item for the whole dish with confidence "low".
Return only JSON with an items array: name, quantity, unit, kcal, protein_g, carbs_g, fat_g, confidence (high/medium/low), note (optional).`;
